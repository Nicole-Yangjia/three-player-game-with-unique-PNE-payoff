...
import csv, itertools, re, sys, time, random
...
def check_one_omega(line: str):
    print(f"Testing Omega: {line}")
    parsed = parse_omega_line(line)

    print("\n=== STEP 1 ===")
    print("Checking Σ_G ⊧ Ω ⊃ ¬NE(sx,sy,sz)")
    ...
    if sat:
        print("Result: Σ_G ∧ Ω ∧ NE(sx,sy,sz) is SATISFIABLE")
        print("Conclusion: Σ_G ⊭ Ω ⊃ ¬NE(sx,sy,sz)")
        print("Ω may have PNE - proceeding to Step 2")

        print("\n=== STEP 2 ===")
        print("Checking Σ_G ⊧ Ω ⊃ λ")
        ...
        if sat:
            print("Result: Σ_G ∧ Ω(p1) ∧ Ω(p2) ∧ NE(p1) ∧ NE(p2) ∧ ¬λ is SATISFIABLE")
            print("Conclusion: Σ_G ⊭ Ω ⊃ λ")
            print("Ω does NOT guarantee unique NE")
        else:
            print("Result: Σ_G ∧ Ω(p1) ∧ Ω(p2) ∧ NE(p1) ∧ NE(p2) ∧ ¬λ is UNSATISFIABLE")
            print("Conclusion: Σ_G ⊧ Ω ⊃ λ")
            print("Ω guarantees unique NE")
    else:
        print("Result: Σ_G ∧ Ω ∧ NE(sx,sy,sz) is UNSATISFIABLE")
        print("Conclusion: Σ_G ⊧ Ω ⊃ ¬NE(sx,sy,sz)")
        print("Ω has no PNE - stopping at Step 1")


if __name__ == '__main__':
    rows = list(csv.reader(open('generate_combination_sat.csv', newline='', encoding='utf-8')))
    ...
    if len(sys.argv) > 1 and sys.argv[1].isdigit():
        ...
        if row_num < start or row_num >= len(rows):
            print(f"Row number {row_num} is out of range. Using random row.")
            row_num = random.randint(start, len(rows) - 1)
    else:
        row_num = random.randint(start, len(rows) - 1)

    line = rows[row_num][0].strip()
    print(f"Testing row {row_num}:")

    check_one_omega(line)